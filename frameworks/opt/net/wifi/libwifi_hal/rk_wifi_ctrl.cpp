...
// # RADXA PATCH
						...
						char module_file[256] = {0};
						FILE *module_fp = NULL;
						sprintf(module_file, "%s/%s/module", bus_dir, next->d_name);
						if (0 == strncmp(temp, "02d0:a9bf", 9)) { // check AzureWave Module
							module_fp = fopen(module_file, "r");
							if (NULL != module_fp) {
								fgets(module_line, sizeof(module_line), module_fp);
								PLOG(DEBUG) << "sdio module :" << line << ".";
								// 0x8101 is a magic number. See kernel/drivers/mmc/core/sdio.c
								if (0 == strncmp(module_line, "0x8101", 6)) {
									strcpy(recoginze_wifi_chip, "CM256");
								} else {
									strcpy(recoginze_wifi_chip, supported_wifi_devices[i].wifi_name);
								}
							}
